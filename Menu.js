...
function onOpen() {
  ...
  const planningMenu = ui.createMenu('📋 PLANNING Tool');

  // PRIMARY UPDATE ACTIONS
  planningMenu.addItem('🔄 Update Value Stream OR Scrum Team...', 'menuPrimaryUpdate');
  planningMenu.addSeparator();

  // Analysis and Reporting
  planningMenu.addSubMenu(ui.createMenu('📊 Analysis & Reports')
      .addItem('🎯 Analyze PI (with Destinations)...', 'menuAnalyzePICustom')
      .addSeparator()
      .addItem('📈 Full Update (With Summaries)', 'menuAnalyzeWithSummaries')
      .addItem('⚡ Fast Update (Skip Summaries)', 'menuAnalyzeWithoutSummaries')
      .addItem('❓ Update (Ask About Summaries)', 'menuAnalyzeWithSummaryChoice')
    .addItem('Generate Dans Report', 'generateDansReport')
    .addSeparator()
    .addItem('📈 View PI Planning Dashboard', 'menuShowPIPlanningDashboard'));

  planningMenu.addSeparator();

  // Advanced Operations
  planningMenu.addSubMenu(ui.createMenu('🚀 Advanced Operations')
  ...
  // Utilities submenu
  planningMenu.addSubMenu(ui.createMenu('🔧 Utilities')
    ...
    .addItem('🔍 Diagnostic Check', 'diagnosticCheckDansReportData')
    .addSeparator()
...
function updateFilterMenu() {
  ...
  const filterMenu = ui.createMenu('🔍 Quick Filters');

  // If on a summary sheet, add direct filter options
  if (sheetName.includes('Summary')) {
    filterMenu.addItem('⚠️ LOE Exceeding Estimate', 'instructLOEFilter');
    filterMenu.addItem('🔄 Allocation Mismatches', 'instructMismatchFilter');
    filterMenu.addItem('📊 All Epics', 'instructAllEpicsFilter');

    // Add Stories filter for scrum team summaries
    if (!sheetName.includes(' - ') || sheetName.split(' - ').length > 2) {
      filterMenu.addItem('📝 Stories/Tasks by Status', 'instructStoriesFilter');
    }

    filterMenu.addSeparator();
    filterMenu.addItem('❌ Clear All Filters', 'clearSheetFilters');
  } else {
    // If not on a summary sheet, add navigation options
    filterMenu.addItem('⚠️ Apply LOE Filter...', 'selectSummaryForLOEFilter');
    filterMenu.addItem('🔄 Apply Allocation Mismatch Filter...', 'selectSummaryForMismatchFilter');
    filterMenu.addItem('📊 Apply All Epics Filter...', 'selectSummaryForAllEpicsFilter');
    filterMenu.addSeparator();
    filterMenu.addItem('📋 Go to Summary Sheet...', 'navigateToSummarySheet');
  }

  filterMenu.addSeparator();
  filterMenu.addItem('❓ Filter Instructions', 'showFilterInstructions');
...
function menuBatchUpdateTeams() {
  ...
  const html = HtmlService.createHtmlOutput(`
    ...
    <div class="warning">
      ⚠️ Note: Updating multiple teams may take several minutes depending on the amount of data.
    </div>
...
function menuClearCache() {
  ...
  const response = ui.alert(
    ...
    '• All Caches - Remove all cached data\n' +
    '• Specific PI - Clear cache for a specific PI\n' +
    '• Cancel - Keep existing cache',
  ...
  if (response === ui.Button.YES) {
    ...
    try {
      ...
      ui.alert('✅ All caches cleared successfully!');
    } catch (error) {
      ui.alert('❌ Error clearing cache: ' + error.message);
    }
    ...
    if (piResponse.getSelectedButton() === ui.Button.OK) {
      ...
      if (piNumber && /^\d+$/.test(piNumber)) {
        try {
          ...
          ui.alert(`✅ Cache cleared for PI ${piNumber}!`);
        } catch (error) {
          ui.alert('❌ Error clearing cache: ' + error.message);
        }
      }
...
function instructLOEFilter() {
  ...
  const processingToast = SpreadsheetApp.getActiveSpreadsheet().toast('⏳ Applying LOE filter...', 'Processing', -1);
  ...
  try {
    ...
    SpreadsheetApp.getActiveSpreadsheet().toast('✅ LOE filter applied successfully!', 'Complete', 3);
...
function instructMismatchFilter() {
  ...
  // Show immediate processing feedback
  const processingToast = SpreadsheetApp.getActiveSpreadsheet().toast('⏳ Applying Allocation Mismatch filter...', 'Processing', -1);
  ...
  try {
    ...
    // Clear the processing toast and show success
    SpreadsheetApp.getActiveSpreadsheet().toast('✅ Allocation Mismatch filter applied successfully!', 'Complete', 3);
...
function instructAllEpicsFilter() {
  ...
  // Show immediate processing feedback
  const processingToast = SpreadsheetApp.getActiveSpreadsheet().toast('⏳ Applying All Epics filter...', 'Processing', -1);
  ...
  try {
    ...
    // Clear the processing toast and show success
    SpreadsheetApp.getActiveSpreadsheet().toast('✅ All Epics filter applied successfully!', 'Complete', 3);
...
function instructStoriesFilter() {
  ...
  // Show immediate processing feedback
  const processingToast = SpreadsheetApp.getActiveSpreadsheet().toast('⏳ Applying Stories filter...', 'Processing', -1);
  ...
  try {
    ...
    // Clear the processing toast and show success
    SpreadsheetApp.getActiveSpreadsheet().toast('✅ Stories filter applied successfully!', 'Complete', 3);
...
function clearSheetFilters() {
  // Show immediate processing feedback
  const processingToast = SpreadsheetApp.getActiveSpreadsheet().toast('⏳ Clearing filters...', 'Processing', -1);
  ...
  try {
    ...
    if (filter) {
      filter.remove();
      SpreadsheetApp.getActiveSpreadsheet().toast('✅ All filters have been cleared!', 'Complete', 3);
    } else {
      SpreadsheetApp.getActiveSpreadsheet().toast('ℹ️ No active filters to clear on this sheet.', 'Info', 3);
    }
  } catch (error) {
    SpreadsheetApp.getActiveSpreadsheet().toast('', '', 1); // Clear toast
    console.error('Error clearing filters:', error);
    SpreadsheetApp.getActiveSpreadsheet().toast('⚠️ Error clearing filters: ' + error.toString(), 'Error', 5);
  }
}
...
• Use the filter options to quickly filter specific sections
• Each filter targets a specific data table in the summary
• Clear All Filters removes any active filters

When on Other Sheets:
• Select a filter option to choose which summary sheet to apply it to
• Use "Go to Summary Sheet" to navigate to a summary sheet first

Filter Types:
⚠️ LOE Exceeding Estimate - Shows epics where LOE > Story Point Estimate
🔄 Allocation Mismatches - Shows epics with allocation conflicts
📊 All Epics - Shows all epics with sorting options

Tips:
• Filters are applied to specific sections only
• Each section maintains its own filter state
...
function UpdateScrumTeamsWithSummaries(piNumber, scrumTeams) {
  ...
  try {
    ...
    if (successCount > warningCount) {
      message += `✓ Team summary sheets created/updated for teams with data\n`;
    }
